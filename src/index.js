...
healthServer.listen(PORT, "0.0.0.0", () => {
    console.log(chalk.blue(chalk.bold("Web")), chalk.white(">>"), chalk.green(`LightCore health server listening on port ${PORT}`));
});
...
const makeWebhook = (name) => {
    ...
    try {
        ...
        console.warn(`Invalid ${name} webhook configuration:`, error.message);
        return null;
    }
};
...
if (!process.env.DISCORD_ID) {
    console.error("Missing required environment variable: DISCORD_ID");
    process.exit(1);
}
...
if (process.env.TOPGG_TOKEN) {
    ...
    const postMetrics = async () => {
        try {
            ...
            await topgg.postMetrics({
                serverCount: counts.reduce((a, b) => a + b, 0) || 0,
    ...
    setTimeout(async () => {
        try {
            const commands = await manager.broadcastEval(
                async (client) => (await client.application.commands.fetch()).map((command) => command.toJSON()),
                { shard: 0 },
            );
...
manager.on("shardCreate", (shard) => {
    ...
    safeWebhookSend(startLogs, {
        username: "LightCore Logs",
        embeds: [new Discord.EmbedBuilder()
            .setTitle("🆙・Launching shard")
            .setDescription(`Shard **${shardLabel}** is starting.`)
    ...
    shard.on("death", (process) => {
        const embed = new Discord.EmbedBuilder()
            .setTitle("🚨・Shard exited")
            .setDescription(`Shard **${shardLabel}** exited unexpectedly.`)
            .addFields(
                { name: "PID", value: `\`${process.pid}\``, inline: true },
                { name: "Exit code", value: `\`${process.exitCode ?? "unknown"}\``, inline: true },
            )
            .setColor(config.colors.error)
            .setTimestamp();
        safeWebhookSend(shardLogs, { username: "LightCore Logs", embeds: [embed] });
    });

    shard.on("shardDisconnect", (event) => {
        safeWebhookSend(shardLogs, {
            username: "LightCore Logs",
            embeds: [new Discord.EmbedBuilder()
                .setTitle("🚨・Shard disconnected")
                .setDescription(`Shard **${shardLabel}** disconnected.`)
                .addFields({ name: "Close event", value: `\`${String(event?.code ?? "unknown")}\`` })
                .setColor(config.colors.error)
                .setTimestamp()],
        });
    });
    ...
    shard.on("shardReconnecting", () => {
        safeWebhookSend(shardLogs, {
            ...
            embeds: [new Discord.EmbedBuilder()
                ...
                .setDescription(`Shard **${shardLabel}** is reconnecting.`)
                .setColor(config.colors.normal)
                .setTimestamp()],
        });
    });
...
manager.spawn().catch((error) => {
    ...
    safeWebhookSend(consoleLogs, {
        username: "LightCore Logs",
        embeds: [new Discord.EmbedBuilder().setTitle("🚨・Shard manager failed").setDescription(`\`\`\`${String(error).slice(0, 3800)}\`\`\``).setColor(config.colors.error)],
    });
});

process.on("unhandledRejection", (error) => {
    console.error("Unhandled promise rejection:", error);
    const text = String(error?.stack || error).slice(0, 1900);
    safeWebhookSend(consoleLogs, {
        username: "LightCore Logs",
        embeds: [new Discord.EmbedBuilder()
...
process.on("warning", (warn) => {
    ...
    safeWebhookSend(warnLogs, {
        username: "LightCore Logs",
        embeds: [new Discord.EmbedBuilder()
            .setTitle("⚠️・Node.js warning")
            .setDescription(`\`\`\`${String(warn).slice(0, 3800)}\`\`\``)
            .setColor(config.colors.normal)
            .setTimestamp()],
    });
});
