...
// Explicit command routing prevents keyword collisions such as `profile` being
// classified as Economy just because its name contains a broad keyword.
...
function buildCommandLines(client, category, mode, prefix) {
  return loadedCommands(client).filter(e => categoryForName(e.name, e.parent) === category).sort((a, b) => a.name.localeCompare(b.name)).map(e => {
...
function buildEmbed(client, mode, category) {
  const prefix = client.config.discord.prefix || ".", lines = buildCommandLines(client, category, mode, prefix), chunks = [];
  let chunk = "";
  for (const line of lines) {
    const next = chunk ? `${chunk}\\n${line}` : line;
    if (next.length > 1000) { if (chunk) chunks.push(chunk); chunk = line; } else chunk = next;
  }
...
function buildMenu(mode, owner) {
  return new Discord.ActionRowBuilder().addComponents(new Discord.StringSelectMenuBuilder().setCustomId(mode === "prefix" ? `lc_phelp:${owner}` : `lc_help:${mode}:${owner}`).setPlaceholder(mode === "prefix" ? "Choose a prefix help category" : "Choose a LightCore help category").addOptions(CATEGORY_ORDER.map(c => ({ label: LABELS[c][0], description: `View ${LABELS[c][0].toLowerCase()} ${mode} commands`, emoji: LABELS[c][1], value: c }))));
}
